...
export interface FilterSidebarProps {
  ...
  onWeekendChange?: (weekend: string) => void;
...
const PROPERTY_TYPES = [
  { key: "APARTMENT", labelKey: "filters.prop_types.apartment", fallback: "Apartment" },
  { key: "VILLA", labelKey: "filters.prop_types.villa", fallback: "Villa" },
  { key: "PENTHOUSE", labelKey: "filters.prop_types.penthouse", fallback: "Penthouse" },
  { key: "STUDIO", labelKey: "filters.prop_types.studio", fallback: "Studio" },
];
...
export default function FilterSidebar({
  ...
  const { t } = useLanguage();
  const { data: weekendCalendarsData } = useWeekendCalendars({ limit: 50 });

  const totalActiveFilters =
    selectedAmenities.length +
    selectedPropertyTypes.length +
    (selectedWeekend && selectedWeekend !== "any" ? 1 : 0);

  // Dynamic Shabbat calendar options from backend
  const weekendOptions = [
    ...
    ...(weekendCalendarsData?.data && Array.isArray(weekendCalendarsData.data)
      ? [...weekendCalendarsData.data]
  ...
  const selectedWeekendLabel =
    weekendOptions.find((opt) => opt.value === selectedWeekend)?.label ||
        ...
        <DropdownMenu>
          <DropdownMenuTrigger className="w-full flex items-center justify-between pl-3 pr-2 py-2 bg-zinc-50 dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-xl text-xs font-semibold text-zinc-700 dark:text-zinc-300 outline-none focus:ring-2 focus:ring-[#4c55a4] transition-all cursor-pointer">
            <div className="flex items-center gap-1.5 truncate">
              ...
              <span className="truncate">{selectedWeekendLabel}</span>
            </div>
            <ChevronDown className="h-3.5 w-3.5 text-zinc-400 shrink-0 opacity-80" />
          </DropdownMenuTrigger>
          <DropdownMenuContent align="start" className="w-[240px] max-h-[300px] overflow-y-auto rounded-xl bg-white dark:bg-zinc-950 border border-zinc-200 dark:border-zinc-800 p-1.5 shadow-2xl z-50">
            {weekendOptions.map((opt) => (
              <DropdownMenuItem
                key={opt.value}
                onClick={() => onWeekendChange?.(opt.value)}
              ...
              >
                ...
                {selectedWeekend === opt.value && <Check className="w-3.5 h-3.5 text-[#4c55a4] shrink-0" />}
              </DropdownMenuItem>
            ))}
          </DropdownMenuContent>
        </DropdownMenu>
      ...
      <div className="space-y-4 mb-6">
        ...
        <div className="grid grid-cols-2 gap-2">
          {PROPERTY_TYPES.map((typeObj) => {
            const isChecked = selectedPropertyTypes.includes(typeObj.key);
            return (
              <label key={typeObj.key} className="flex items-center gap-3 cursor-pointer group select-none">
                <div className="relative flex items-center justify-center shrink-0">
                  <input
                    type="checkbox"
                    checked={isChecked}
                    onChange={() => onPropertyTypeToggle?.(typeObj.key)}
                ...
                <span className={`text-sm font-medium transition-colors ${isChecked ? 'font-bold text-[#4c55a4] dark:text-indigo-400' : 'text-zinc-700 dark:text-zinc-300 group-hover:text-zinc-900 dark:group-hover:text-white'}`}>
                  {t(typeObj.labelKey) || typeObj.fallback}
                </span>
              </label>
            );
          })}
