...
export default function CreateListingModal({
  ...
  useEffect(() => {
    if (isOpen && (isEditMode || initialApartment)) {
      ...
      setEmail(initialApartment?.contactEmail || meUser?.email || "");
      setWhatsapp(initialApartment?.whatsApp || meUser?.phone || "");
      ...
      setRentFrequency(initialApartment?.howOftenWantToRent || initialApartment?.rentFrequency || "");
  ...
  const handleSaveAndContinue = async () => {
    ...
    if (whatsappEnabled && whatsapp) formData.append("whatsApp", whatsapp);
    // Removed contactEmail as it causes a Prisma validation error on the backend (Unknown argument).
    // if (emailEnabled && email) formData.append("contactEmail", email);
    ...
    if (rentFrequency) formData.append("howOftenWantToRent", rentFrequency);
                            ...
                            <div className="p-1.5 max-h-64 overflow-y-auto divide-y divide-zinc-100 dark:divide-zinc-800/50">
                              {PROPERTY_TYPES.map((pt) => (
                                <button
                                  ...
                                  className={`w-full text-left px-3.5 py-2.5 rounded-xl cursor-pointer transition-colors flex items-center justify-between text-sm ${propertyType === pt
                ...
                <div className="bg-white dark:bg-zinc-900 rounded-3xl border border-zinc-200 dark:border-zinc-800 p-8 shadow-sm shadow-zinc-200/50 dark:shadow-none">
                  ...
                  <div className="grid grid-cols-2 sm:grid-cols-4 lg:grid-cols-5 gap-4">
                    {galleryPreviews.map((preview, index) => (
                      <div key={index} className="aspect-square relative overflow-hidden border border-zinc-200 dark:border-zinc-800 rounded-2xl flex flex-col items-center justify-center group bg-zinc-50 dark:bg-zinc-950 shadow-sm hover:shadow-md transition-shadow">
                        ...
                        <div className="absolute inset-0 bg-black/40 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center">
                          <button
                            ...
                            className="bg-white/20 hover:bg-red-500 backdrop-blur-md text-white p-2.5 rounded-full shadow-lg transition-all scale-90 group-hover:scale-100"
                            title="Remove Photo"
                          >
                            <X className="w-5 h-5" />
                          </button>
                            ...
                            <div className="p-2 max-h-64 overflow-y-auto divide-y divide-zinc-100 dark:divide-zinc-800/50">
                              {RENT_FREQUENCY_OPTIONS.map((opt) => (
                                <button
                                  ...
                                  className={`w-full text-left px-4 py-3 rounded-xl cursor-pointer transition-colors flex items-center justify-between text-sm ${rentFrequency === opt.value
                                      ? "bg-[#4c55a4]/10 dark:bg-indigo-900/30 text-[#4c55a4] dark:text-indigo-400 font-bold"
                                      : "hover:bg-zinc-100 dark:hover:bg-zinc-800 text-zinc-800 dark:text-zinc-200 font-medium"
                                    }`}
                                >
