...
import { User, Menu, Settings, LogOut, LayoutDashboard, ChevronDown, Bell, Globe, Building, X } from "lucide-react";
    ...
    <>
      <header className="sticky top-0 z-50 w-full bg-white/80 backdrop-blur-md border-b border-zinc-100 dark:bg-zinc-950/80 dark:border-zinc-800">
        <div className="container mx-auto px-4 py-4 flex items-center justify-between">
          ...
          {/* Mobile Menu Button */}
          <button onClick={() => setIsMobileMenuOpen(true)} className="lg:hidden p-2 text-zinc-600 dark:text-zinc-300 hover:bg-zinc-100 dark:bg-zinc-900 hover:bg-zinc-200 dark:hover:bg-zinc-800 rounded-lg">
      ...
      {isMobileMenuOpen && (
        <div className="fixed inset-0 z-50 lg:hidden">
          ...
          <div className="fixed top-0 right-0 h-full w-[85%] max-w-sm bg-white dark:bg-zinc-950 shadow-2xl overflow-y-auto animate-in slide-in-from-right duration-300 border-l border-zinc-200 dark:border-zinc-800">
            <div className="p-5 border-b border-zinc-100 dark:border-zinc-800 flex items-center justify-between sticky top-0 bg-white/90 dark:bg-zinc-950/90 backdrop-blur z-10">
              ...
              <button onClick={() => setIsMobileMenuOpen(false)} className="p-2 text-zinc-500 hover:text-zinc-900 dark:hover:text-white bg-zinc-100 dark:bg-zinc-900 hover:bg-zinc-200 dark:hover:bg-zinc-800 rounded-full transition-colors">
                <X className="w-5 h-5" />
              </button>
            </div>
            ...
            <div className="p-5 flex flex-col gap-3">
              {navLinks.map((link) => {
                ...
                return (
                  <Link
                    ...
                    className={`font-bold text-lg transition-colors py-3 px-4 rounded-xl ${isActive ? "bg-blue-50 text-blue-600 dark:bg-blue-900/20 dark:text-blue-400" : "text-zinc-900 dark:text-white hover:bg-zinc-50 dark:hover:bg-zinc-900"}`}
                  >
                    {link.label}
                  </Link>
                );
              ...
              <div className="mt-4 pt-6 border-t border-zinc-100 dark:border-zinc-800 flex flex-col gap-4">
                ...
                <div className="flex items-center justify-between mb-2 px-2">
                   <span className="text-sm font-bold text-zinc-500 dark:text-zinc-400 flex items-center gap-2"><Globe className="w-4 h-4"/> Language</span>
                   <div className="flex gap-2 bg-zinc-100 dark:bg-zinc-900 p-1 rounded-xl">
                     <button onClick={() => setLanguage("EN")} className={`px-4 py-1.5 rounded-lg text-sm font-bold transition-all ${language === "EN" ? "bg-white dark:bg-zinc-800 text-[#4c55a4] dark:text-indigo-400 shadow-sm" : "text-zinc-500 hover:text-zinc-700 dark:hover:text-zinc-300"}`}>EN</button>
                     <button onClick={() => setLanguage("HE")} className={`px-4 py-1.5 rounded-lg text-sm font-bold transition-all ${language === "HE" ? "bg-white dark:bg-zinc-800 text-[#4c55a4] dark:text-indigo-400 shadow-sm" : "text-zinc-500 hover:text-zinc-700 dark:hover:text-zinc-300"}`}>HE</button>
                   </div>
                </div>
                ...
                {!userRole ? (
                  <>
                    <button 
                      onClick={() => { setIsMobileMenuOpen(false); setIsAddModalOpen(true); }}
                      className="w-full bg-gradient-to-r from-[#4c55a4] to-[#6b75c8] hover:opacity-90 text-white py-3.5 rounded-xl font-extrabold shadow-md shadow-[#4c55a4]/20 flex items-center justify-center gap-2"
                    >
                  ...
                  <>
                    ...
                    {isOwner && (
                      <Link 
                        href="/user-dashboard"
                        onClick={() => setIsMobileMenuOpen(false)}
                        className="w-full bg-gradient-to-r from-[#4c55a4] to-[#6b75c8] text-white py-3.5 rounded-xl font-extrabold flex items-center justify-center gap-2 shadow-md shadow-[#4c55a4]/20"
                      >
                        <Building className="w-4 h-4" /> Manage My Apartment
                      </Link>
                    )}
                    <Link 
                      href={userRole === "admin" ? "/dashboard" : "/user-dashboard"}
                    ...
                    <button
                      ...
                      className="w-full bg-red-50 dark:bg-red-500/10 hover:bg-red-100 dark:hover:bg-red-500/20 text-red-600 dark:text-red-400 py-3.5 rounded-xl font-extrabold flex items-center justify-center gap-2 transition-colors"
                    >
